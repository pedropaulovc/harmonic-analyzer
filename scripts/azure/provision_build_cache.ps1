...
Write-Host ""
Write-Host "Account / container / salt are committed defaults in _artifact_cache.py, so a"
Write-Host "machine only declares its ROLE -- once, no persistent env var needed:"
Write-Host "    Set-Content .harmonic-cache-mode rw   # builder (vm-solidworks): pull + push"
Write-Host "    Set-Content .harmonic-cache-mode ro   # dev box: pull only"
Write-Host "(gitignored; HARMONIC_CACHE_MODE env overrides it if set.)"
Write-Host ""
