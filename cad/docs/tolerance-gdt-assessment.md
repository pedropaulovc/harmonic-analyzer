...
4. **The Ø6-tooth gear is marginal by construction.** After a forced rebuild, `build_cone_gear.py`
   measures the T006 base-chord root at 1.432434 mm radius and enforces a **0.611 mm minimum web** at
   the maximum allowed 1/16 in bore (0.639 mm nominal). That is a real machining hazard, not a CAD
...
| **Cone gears** `cone-gear`, `cone-gear-shaft` | **T1** | **bore-to-pitch runout** ≤0.05 mm; **DP 49.82 / 14.5° PA shared across the whole cone↔cylinder train** (its own domain — Finding 3); flag **T006 0.639 mm nominal / 0.611 mm minimum web** + harder tip metal (Finding 4); `precision` on bore. Mesh itself stays **loose** (oblique, adjustable centre distance). |
