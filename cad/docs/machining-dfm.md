...
- **Three parts carry the whole risk. Everything else is generous.**
  1. **`cone-gear` T006** — root-to-bore wall **0.54 mm** on a **~4.08 mm OD / Ø1.588 mm bore** gear.
     The single hardest part in the machine. (Wall measured against the AS-CUT root: these teeth are
     through-cut from the module's own DXF profile, whose gap floor is the chord across the base
     circle, so T006's minimum-material radius is 1.3365 mm, not the 0.939 mm a standard full-depth
     cutter would reach. Against the full-depth root the same bore would leave only 0.15 mm, so the
     cutter profile is load-bearing here. Every small cone gear is sub-1 mm: T012 0.83, T018 0.78,
     T024 0.71.) **No executable guard enforces this** — it lives only in a comment. Period
     mitigation: the tip gears were a harder yellow metal.
...
1. **Add an executable wall guard for the small cone gears.** The 0.54 mm T006 wall (and 0.71–0.83
...
- **DFMPro (SW add-in) and Xometry auto-DFM — false-green here (observed).** Both passed *every* SLDPRT
  ...
  cannot cut, and sub-mm walls (T006 **0.54 mm**). Their rule sets are tuned for moulding / sheet /
...
- **Fusion 360 CAM verify/simulate — the one worth banking on, with a known edge.** Because it
  ...
  - **But its blind spot is exactly where the worst risk lives.** Fusion treats geometry as **rigid**
    and stock as **held**, so it will *not* warn that the **T006 0.54 mm wall** breaks in workholding,
