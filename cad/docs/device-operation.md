...
|---|---|---|---|
| setup | **lubricate the drum drive**: Lubricate each connecting-rod strap on its cam and each drum arbor in its pedestal with castor oil or grease before running, and keep them lubricated; never run them dry. | the channel spring's torque on each drum reverses twice a turn; against lubricated brass on steel (μ 0.11–0.19) the drum overruns onto its other flank in the releasing half, and the error budget scores exactly that (the `flank_toggle` term, modelled, not proven). Dry (μ 0.44–0.51) is a different machine the budget does not score | Engineering ToolBox friction table (brass on steel: castor oil 0.11, grease 0.19); `error_budget.yaml` `flank_toggle`; [`tolerance-policy.md`](./tolerance-policy.md) |
...
| 2 | crank in one direction; read the pen height $r_k$ (mm) off the trace's own mean line at $\theta_k$, i.e. with the crank on its index after $2k$ turns. If you overshoot a crank index, go on round to it again; never back the crank off. | the coefficients are the ordinates at those divisions; backing off re-seats every cone-drum mesh on its other flank | 1898 p. 10; book p. 8 |
