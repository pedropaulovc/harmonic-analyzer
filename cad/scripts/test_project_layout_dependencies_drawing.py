...
def test_common_framework_has_no_transitive_native_layout_dependency():
    closure = {Path(path).name for path in module_deps_of(SCRIPTS / "_drawing_common.py")}
    assert not closure & (NATIVE_LAYOUT_HELPERS | {"_drawing_project_layout.py"})


@pytest.mark.parametrize("script", sorted(SCRIPTS.glob("draw_*.py")), ids=lambda p: p.name)
def test_only_explicit_native_layout_pilots_include_the_full_native_helper_closure(script):
