...
BA6 = TapSpec("6 BA", 2.80, 0.53, 47.5, 2.30)
...
async def add_cosmetic_ba_threads(
    ...
    *,
    ...
    for index, (x_mm, y_mm) in enumerate(points_xy):
        edge_point = [
            x_mm + tap.tap_drill_diameter_mm / 2.0,
            y_mm,
            z_face_mm,
        ]
...
async def create_tapped_pattern(
    ...
    *,
    ...
    check(f"create_sketch {name} tap drills", await adapter.create_sketch("Front"))
    ...
    for index, (x_mm, y_mm) in enumerate(points_xy):
        await define_circle(
            ...
            x_mm,
            y_mm,
            tap.tap_drill_diameter_mm / 2.0,
            f"{tap.designation} tap drill ({x_mm:g}, {y_mm:g})",
            dims=dimensions,
            names=(f"{name}P{index}X", f"{name}P{index}Y", f"{name}P{index}Dia"),
        )
    set_sketch_direct_db(adapter, False)
    await ensure_fully_defined(adapter, f"{name} tap-drill sketch")
    check(f"exit_sketch {name} tap drills", await adapter.exit_sketch())
    profile_name = f"{name}TapDrillProfile"
    name_last_feature(adapter, profile_name)
    dimensions.apply(adapter, profile_name)
    check(
        f"cut {name} tap drills",
    ...
    feature_name = f"{name}TapDrills"
    name_last_feature(adapter, feature_name)
    feature = adapter.currentModel.FeatureByName(feature_name)
    if feature is None:
        raise RuntimeError(f"created tap-drill feature {feature_name!r} is missing")
    ...
    return CreatedTapPattern(
        feature=feature,
        tap_drill_diameter_mm=tap.tap_drill_diameter_mm,
