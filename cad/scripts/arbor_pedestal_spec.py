...
DRAWING_NOTES = "\n".join(
    (
        ...
        "DATUM A IS FOOT SEAT. ARBOR BORE AXIS CONTROL IS DIA 0.05",
        "CYLINDRICAL ZONE PARALLEL TO A; FINISH RA 1.6.",
        "DIA 20.00 DOME AND ARBOR BORE ARE CONCENTRIC. STRAIGHT TAPER",
        "RUNS FROM 24.00 WIDTH AT A TO 20.00 WIDTH AT BORE CENTERLINE;",
        "TAPER TERMINATES AT DOME HORIZONTAL DIAMETER, NOT TANGENT.",
        "STRAP 10.00 +/-0.10 THICK; +Z STRAP FACE FLUSH WITH +Z FOOT FACE;",
        "6.00 +/-0.10 EXPOSED FLANGE REMAINS ON -Z SIDE.",
        f"FLANGE HOLE DIA 3.264 +0.10/-0.00 ({SCREW_THREAD} NORMAL CLEARANCE)",
        "THRU; AXIS ON 24.00 WIDTH CENTERLINE +/-0.05 AND 5.00 +/-0.10",
        "FROM FOOT CENTER PLANE TOWARD -Z EXPOSED FLANGE.",
    )
)
