...
def test_linked_notes_define_remaining_turned_part_operations() -> None:
    ...
    assert "DRILL 1/16 IN (1.588) BORE THRU" in notes
    assert "Depth" not in drawing.DIMENSION_CALLOUTS
    assert drawing.DIMENSION_CALLOUTS["BoreDiaDim"] == "1/16 IN THRU"
    # The bore rides the cone shaft's 1/16 in tip journal line-to-line at nominal;
    # the +0.05/-0.00 drilled callout keeps it a slip fit, never an interference.
    assert cone_tip_bushing_spec.BORE_DIA == 0.0625 * 25.4
