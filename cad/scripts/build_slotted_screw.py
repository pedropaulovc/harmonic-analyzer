r"""Reproduction script: rig hold-down slotted screw (book ch. 25; 5 used).

The plain slotted machine screw that bolts the alignment-pinion rig's
base hardware down (p. 69 block close-up ``page002_img01``): two per
pivot block (the bright pair) and one through the return spring's foot
(the dark one at frame left -- same part, tinted per instance like the
cone tip gears). Head bears on the part's top face, shank drops through
...
HEAD_H = 2.5
SHANK_DIA = 4.0  # rides the Ø4.2 holes in block/spring foot (derived)
SHANK_LEN = 18.0  # through the 16-tall block + 2 engagement into the base;
# the spring-foot instance buries the surplus in the base (thread depth
# unmodeled, the pedestal-bolt precedent)
