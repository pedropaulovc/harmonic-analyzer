...
sketch-text API in the adapter, so -- the nameplate precedent -- they come from
a vendored DXF of closed loops, ``cad/references/measuring-stick-numerals.dxf``,
GENERATED by ``gen_stick_numerals_dxf.py`` from this script's constants (font
rendering via ``_dxf_text``), imported onto the Front plane at scale 1 /
position (0, 0) (placement is baked into the file: the Makers seat ignores the
importer's scale/position, see ``build_nameplate``) and cut ``TICK_DEPTH`` deep
as one feature. The frame/sign conventions (the z=0 ruled face is SEEN from
-Z, so the file carries a y inversion, never applied at import) are worked out
in the generator's docstring. Edit a ``NUMERAL_*`` knob here -> re-run the
generator -> re-pin ``NUMERAL_AREA_MM2`` / ``NUMERALS_BBOX`` from its output.

Known, accepted (2026-09-02 pass 3): the tick layout is the MIRROR image of the
photographed stick. page001_img01 shows 0 at the left with the ticks hanging from
the FAR long edge; this model's ruled face, seen face-on with 0 at the left (the
drawing's rotated ``*Back`` view), has them hanging from the NEAR edge, because
the tick sketch was drawn in the +Z frame and cut into the z=0 back face. A
plain bar has no other handedness, so it only matters once the stop block's
orientation is tied to the ruled edge; the numerals are laid out in the
as-seen (-Z) frame so they read correctly on the face regardless.
...
# Engraved numerals 0..10 (see module docstring). The literal filename below is
# load-bearing: _buildgraph.data_deps_of scans this script for quoted *.dxf
# names to make the DXF a file_dep of the part (an edit to the file rebuilds it).
...
# the file back without pulling matplotlib into the part's recipe). The BBOX is
# a test-only pin: test_dxf_text (check:numerals) holds both constants to the
# generator's output, so a stale DXF or a forgotten re-pin fails offline first.
NUMERAL_AREA_MM2 = 13.108  # 19 loops: 12 glyph bodies, 7 counters (0 4 6 8 8 9 0)
NUMERALS_BBOX = (58.300, 1.393, 198.700, 4.400)  # x0, y0, x1, y1 (mm, model frame)
...
async def build(adapter) -> dict[str, str]:
    ...
    # Numerals 0..10: import the generated DXF (closed loops at final model mm,
    # already y-inverted to read correctly on the z=0 face seen from -Z -- see
    # gen_stick_numerals_dxf) onto the Front plane and cut TICK_DEPTH into +Z,
    # the same direction and depth as every tick. Imported at identity
    # placement because the Makers seat ignores SetPosition/SetSheetScale
    # (build_nameplate); scale/position are passed as (1, (0, 0)) as a
    # statement of intent, not a control. No dims to drive: the numerals are
    # not parametric, so this feature stays out of drive_jobs and, cut before
    # the v_built capture below, out of the neutrality proof's way.
    if not NUMERALS_DXF.is_file():
        raise RuntimeError(f"numerals DXF not found: {NUMERALS_DXF}")
    pre = await adapter.get_mass_properties()
    if not pre.is_success:
        raise RuntimeError(f"measuring stick: get_mass_properties failed: {pre.error}")
    check(
        "import numerals DXF",
        await adapter.import_dxf_dwg(
            ImportDxfDwgParameters(
                file_path=str(NUMERALS_DXF),
                plane="Front",
                scale=1.0,
                position=[0.0, 0.0],
                merge_points=True,  # closed LWPOLYLINEs -> closed regions
                import_hatch=False,
    ...
    if not post.is_success:
        raise RuntimeError(f"measuring stick: get_mass_properties failed: {post.error}")
    removed = float(pre.data.volume) - float(post.data.volume)
    expected = NUMERAL_AREA_MM2 * TICK_DEPTH
    _telemetry.info(
        f"numerals cut removed {removed:.3f} mm^3 (DXF loops {NUMERAL_AREA_MM2:.3f} mm^2 "
        f"x {TICK_DEPTH} = {expected:.3f})"
    )
    # 25 %: the loops clear every tick (no shared volume, pinned by test_dxf_text),
    # so the cut is area x depth up to the importer's curve fitting and the
    # sketch-region merge; a mirrored/offset import that misses the face, a
    # stale DXF or a wrong plane lands far outside this band.
    if abs(removed - expected) > 0.25 * expected:
        raise RuntimeError(
            f"cut numerals: removed {removed:.3f} mm^3, expected {expected:.3f} "
            f"+/-25% -- check the DXF import plane/placement or re-pin NUMERAL_AREA_MM2"
        )
