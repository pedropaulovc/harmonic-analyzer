...
# active_count build-speed knob): channel = 7N + 8
# (N×{rocker,rod,clevis pin,bar,lever,spring,spring hook} + 2 shafts +
# 2 rocker brackets + 2 fulcrum keepers + 2 keeper foot screws; 2026-09-02:
# spacer bushings are gone; one photographed retaining pin is explicit per rod),
# drive-train = 61 + N
...
_COMPONENT_BAND = {
    ...
    "channel": (
        7 * _N_CH + 8 - 6,
        7 * _N_CH + 8 + 6,
    ),  # N=20 -> (142,154), expected 148
    # The ±6 reference margin follows the other measured subsystem bands; the
    # exact 7N+8 inventory is hard-gated in build_channel_assembly.
...
def verify_amplitude_preset(report: Report) -> None:
    """The amplitude-bar stations in channels.yaml obey the configured preset law.

    The photographed ``fan`` Default divides ``fundamental_station_mm`` evenly
    across the channel rows. ``neutral`` parks every bar at zero; ``square``
    populates odd harmonics with ``fundamental_station_mm / harmonic_n``. Every
    law is checked against the seesaw travel, and truth_model must read the
    same vector the geometry builds.
    """
    ...
    def _law() -> None:
        if preset == "fan":
            ...
            for ch in rows:
                ...
                want = fundamental * int(ch["index"]) / denominator
                _expect(
                    abs(got - want) < 5e-4,
                    f"channel {ch['index']}: amplitude_mm {got} != fan law {want:.4f}",
                )
            return
        ...
        _expect(
            preset == "square",
            f"amplitude preset is {preset!r}, not 'fan'/'square'/'neutral' (update this gate)",
        )
        for ch in rows:
            n = ch["harmonic_n"]
            ...
            _expect(
                abs(got - want) < 5e-4,
                f"channel {ch['index']} (n={n}): amplitude_mm {got} != square law {want:.4f}",
            )
