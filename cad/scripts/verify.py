...
async def _verify_motion_one(adapter: Any, report: Report) -> None:
    ...
    try:
        ...
        for theta in far:
            ...
            try:
                check_no_interference(
                    adapter, allowed_pairs=allowed_interference_pairs(name)
                )
...
def verify_spring_base(report: Report) -> None:
    ...
    def _channel_mounts() -> None:
        ...
        for station in stations:
            pose = mounts.channel_pose(station)
            length = spec.INSTALLED_LENGTH_MM if station == 0.0 else pose.length_mm
            ends = stock.end_centers_mm(length)
            for local, expected in zip(
                ends, (pose.lower_eye_xy, pose.upper_eye_xy), strict=True
            ):
                actual = tuple(
                    pose.centre_xy[k] + local[0] * pose.axis_xy[k] for k in range(2)
                )
                _expect(
...
def verify_base_footprint(report: Report) -> None:
    ...
    def _mounts_on_plate() -> None:
        ...
        corners_local = (
            # The NW corner stands at the I31-derived WEST_HALF_N, the NE at
            # HALF_WIDTH_N.
