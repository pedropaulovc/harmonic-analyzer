...
def test_drawing_places_bom_balloons_and_specific_notes() -> None:
    ...
    assert source.count("add_component_bom_balloons(") == 2
    assert '("transgear-bracket", "12")' in source
    assert '("transgear-latch", "15")' in source
    assert '("rack-pinion", "16")' in source
    assert 'items=(("bracket-screw", "13"),)' in source
