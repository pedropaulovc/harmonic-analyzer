...
# ---- control leaf (diag/tolstack-ctl): baseline vs the production pin ----

_BASELINE: list[tuple[int, str]] = []


def export_baseline(adapter: Any, out_pdf: Path, rebuild: Any) -> None:
    """Export the sheet with the template's zero style, then restore the pin.

    Never raises: a diagnostic failure is logged and the production path
    (which the pin already configured) carries on.
    """
    import traceback
    ...
    try:
        draw = adapter.currentModel
        ext = draw.Extension
        _say(f"prefs at finalize {_read_prefs(ext)}")
        _set_zero(ext, ZERO_SHOW)
        baseline = out_pdf.parent / f"{out_pdf.stem}.tolstack-baseline.pdf"
        _BASELINE[:] = _export(adapter, draw, baseline, "baseline", rebuild)
    except Exception:  # noqa: BLE001 - diagnostic only
        _say("baseline failed:\n" + traceback.format_exc())
    finally:
        try:
            ext = adapter.currentModel.Extension
            _set_zero(ext, ZERO_REMOVE_ONLY_ON_ZERO)
            if int(ext.GetUserPreferenceInteger(PREF_TOL_TRAILING_ZERO, NO_OPTION)) != ZERO_REMOVE_ONLY_ON_ZERO:
                raise RuntimeError("tolstack-ctl could not restore the production zero pin")
            rebuild(adapter, label="tolstack.restore")
        except RuntimeError:
            raise
        except Exception:  # noqa: BLE001 - diagnostic only
            _say("restore check failed:\n" + traceback.format_exc())


def compare_production(out_pdf: Path) -> None:
    """Measure the production PDF and guard it against the baseline. Never raises."""
    import traceback

    try:
        _guard(_BASELINE, _measure(out_pdf, "production"), "production")
    except Exception:  # noqa: BLE001 - diagnostic only
        _say("compare failed:\n" + traceback.format_exc())
