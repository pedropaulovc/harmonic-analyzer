...
# (REF) overall, driver face to tip, stacked below the under-head length so
# the prominent 28.00 no longer reads as the overall: model points on the
# lower half of each end face (the driver-face rim is whole there; the slot
# breaks it beside the axis), horizontal like the lengths above it.
OVERALL_END_POINTS_MM = (
    (0.0, -0.7 * HEAD_DIA / 2.0, -HEAD_H),
    (0.0, -0.7 * SHANK_DIA / 2.0, SHANK_LEN),
)
OVERALL_TEXT_XY = (SIDE_CENTER[0], SIDE_CENTER[1] - _HEAD_HALF - 0.042)
