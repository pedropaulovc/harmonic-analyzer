...
DRAWING_NOTES = "\n".join(
    (
        ...
        "   THESE NOTES GOVERN ALL SIZES ON THIS SHEET.",
        "2. BORE <MOD-DIAM>16.50 +0.05/0.00 THRU TOP + BOTTOM. AXIS",
        "   15.00 +/-0.05 FROM LEFT FACE AND 12.00 +/-0.05 FROM",
        "   FRONT FACE, PERPENDICULAR TO THE BOTTOM FACE WITHIN",
        "   0.05 OVER THE 29.00 HEIGHT.",
        ...
        "4. SUPPLY QTY 1 SCREW PER ASME B18.6.2: 1/4-20 UNC-2A",
        "   X 16.00 UNDER-HEAD, THREADED FULL LENGTH (2P MAX",
        "   INCOMPLETE AT HEAD); 10.00 +/-0.13 SQ X 6.00 +/-0.13",
        "   HEAD, CONCENTRIC WITHIN 0.25; FLAT POINT, 0.4 X 45-DEG",
        "   CHAMFER; AISI 1018, BLACK OXIDE MIL-DTL-13924 CL1.",
        "5. MASK BORE, TAPPED HOLE + BOTTOM FACE DURING COATING.",
    )
)
ISOMETRIC_VIEW_NOTE = (
    "ISOMETRIC VIEW SCALE 1:1\n"
    "SQUARE ON THE FRONT FACE = THE NOTE-4 SCREW HEAD,\n"
    "SHOWN INSTALLED FOR REFERENCE (NOT BLOCK GEOMETRY)"
)
