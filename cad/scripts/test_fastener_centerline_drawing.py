...
@pytest.mark.parametrize(
    "scale,center", [((8, 1), (0.19, 0.19)), ((3, 1), (0.3, 0.08))]
)
def test_factory_resolves_feature_role_independently_of_layout(
    monkeypatch, tmp_path, scale, center
):
    ...
    extra_view = object()
    decorator = Mock(return_value={"right": extra_view})
    layout = Mock()
    recipe = replace(
        RECIPE, scale=scale, side_center=center, decorate=decorator, layout=layout
    )
    result = asyncio.run(
    ...
    centerline.assert_called_once_with(
        adapter, side, entity=face, label="pinch side-view axis centerline"
    )
    decorator.assert_called_once_with(adapter, side, side, side)
    layout.assert_called_once_with(
        adapter,
        {"side": side, "end": side, "iso": side, "right": extra_view},
        {
            "manufacturing": factory.add_property_linked_note.return_value,
            "end-caption": factory.add_property_linked_note.return_value,
        },
    )
