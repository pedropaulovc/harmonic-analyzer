...
under-head length and the axis carries a centerline (rule 7); two lines of
note (rule 6); hidden lines on in the profile.
"""
...
def test_lengths_and_slot_are_marked_model_dimensions() -> None:
    ...
    # The shared slotted-cylindrical-head builder names the extrude depths
    # and the slot dims; the build hands it the spec's marked set.
...
def test_overall_is_a_conspicuous_reference_below_the_lengths() -> None:
    # Blind review: "no conspicuous overall; add (14.50) REF".  The overall is
    # a drawing-native horizontal between the two end faces (model points on
    # the lower half of each face, projected into the view), parenthesised,
    # stacked below the under-head length.
    source = _source()
