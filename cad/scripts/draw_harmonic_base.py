...
# the bank nothing in the frame fixes the rig until the fitter sets it (user
# ruling P1-2), so both callouts name the RIG SET note that states it and the
# spring pad's leaf (RIG_SET_STEP).  Each callout keeps the two lines of the
# accepted pre-P1 render, its first line unchanged: printed on the callouts,
# the step ran through the top border, over the TOP VIEW caption, section
# arrow A and the MHA-132 callout (pc-p1 render).
...
TRANSFER_SPRING_NOTE = f"TRANSFER FROM MHA-114\n{TRANSFER_AFTER_RIG_SET} SIZE AS E1"
# The RIG SET note heads the sheet-2 notes column: under the hole table, above
# the A1-A4 bore note, and short of the 38.1 cross-screw-axis callout.
