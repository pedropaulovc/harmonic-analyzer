r"""McMaster 93075A* low-strength steel hex head screws: the shared family laws.

The laws are the ones ``diag_build_93075A194.py`` reads off the vendor
93075A194 model (its equations and solved sketches) and replays against it
through the replica gate (``diag_build_mcmaster.py 93075A194``).  This module
reproduces them for any size of the family from five catalogue dimensions:
thread major and pitch, length under the head, head width across flats and
head height.

- shank: revolved, with a 45 deg x P*0.851 tip chamfer (their Chamfer1);
- hex head: the across-flats hexagon extruded HH up from the underside;
- corner trim: a Flip=True ThroughAll FeatureCut4 of the inscribed circle
  with a 60 deg draft (the trim cone runs 60 deg from the axis);
- crown dish: a 45 deg revolved cut, r = HW*0.9/2 at the top, HH*0.1 deep;
- thread: a tip-seeded ascending helix of L/P + 1 revolutions (start 90 deg)
  swept with the symmetric UN cutter centred 7P/16 past the tip;
- under-head runout: ONE both-directions FeatureExtrusion3 from a circle
  r = major/2 + 0.0508 on the underside (dir1 down blind 10 with a 20 deg
  shrinking draft, dir2 up 0.254), re-merging the split bodies.
...
# The vendor's under-head runout circle stands this far off the major.
RUNOUT_OVER_MAJOR_R = 0.0508
TIP_CHAMFER_PER_PITCH = 0.851
CROWN_R_PER_HW = 0.9 / 2.0
CROWN_D_PER_HH = 0.1


@dataclass(frozen=True, slots=True)
class HexHeadScrew:
    """Catalogue dimensions of one 93075A* size, in mm."""
    ...
    def underside_y(self) -> float:
        """The vendor origin sits mid-overall."""
        return (self.length - self.head_h) / 2.0
    ...
    def tip_chamfer(self) -> float:
        return self.pitch * TIP_CHAMFER_PER_PITCH

    @property
    def crown_r(self) -> float:
        return self.head_af * CROWN_R_PER_HW

    @property
    def crown_d(self) -> float:
        return self.head_h * CROWN_D_PER_HH
    ...
    @property
    def runout_r(self) -> float:
        return self.major_dia / 2.0 + RUNOUT_OVER_MAJOR_R

    @property
    def head_ac(self) -> float:
        """Across corners of the sharp hexagon, before the corner trim."""
        return self.head_af * 2.0 / math.sqrt(3.0)
...
async def build_hex_head_screw(adapter, dims: HexHeadScrew) -> None:
    ...
    tag = dims.part_no
    major_r = dims.major_dia / 2.0
    pitch = dims.pitch
    h_sharp = pitch * math.sqrt(3.0) / 2.0
    root_r = major_r - 0.75 * h_sharp
    flat_r = dims.head_af / 2.0
    hex_R = flat_r * 2.0 / math.sqrt(3.0)
    underside = dims.underside_y
    tip_y = dims.tip_y
    top_y = dims.top_y
    tip_ch = dims.tip_chamfer
    crown_r = dims.crown_r
    crown_d = dims.crown_d
    ...
    with no_sketch_inference(adapter):
        if sk.CreateCenterLine(0.0, underside / 1000.0, 0.0,
                               0.0, tip_y / 1000.0, 0.0) is None:
            raise RuntimeError(f"{tag} shank: CreateCenterLine failed")
    ...
    v_shank = (math.pi * major_r ** 2 * (dims.length - tip_ch)
    ...
    offset_plane(adapter, "UndersidePlane", underside)
    ...
    v_hex = dims.head_af ** 2 * math.sqrt(3.0) / 2.0 * dims.head_h
    ...
    insert_helix(adapter, pitch, dims.helix_revs, clockwise=False,
    ...
    # without inference off the circle snaps to the major (seen live on 194).
    with no_sketch_inference(adapter):
        if adapter.currentSketchManager.CreateCircleByRadius(
                0.0, 0.0, 0.0, dims.runout_r / 1000.0) is None:
