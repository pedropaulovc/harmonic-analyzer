...
async def main() -> None:
    ...
    print(f"[{path.stem}] NeedsRebuild2 on open = {_nr(model)}")
    ...
    if active is not None:
        ...
        print(f"[cfg] active={_read_member(active,'Name')!r} "
    ...
    print(f"[cfg] configuration names = {names!r}")
    ...
    print(f"[fix] EditRebuild3 = {adapter._attempt(lambda: m2.EditRebuild3(), default=None)!r} "
          f"-> in-mem NeedsRebuild2 = {_nr(m2)}")
    print(f"[fix] Save3(Silent) = {adapter._attempt(lambda: m2.Save3(1,0,0), default=None)!r}")
    m3 = await _fresh(adapter, path)
    print(f"[fix] AFTER EditRebuild3+save, reopen NeedsRebuild2 = {_nr(m3)}")
