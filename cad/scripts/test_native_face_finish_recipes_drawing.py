"""Two whole-cylinder finish calls use native FACE placement, not new equality."""
...
@pytest.mark.parametrize(
    "module, target, label, view_name, entity_name, control_key",
    [
        (
            spring,
            "spring_hook",
...
def test_actual_recipe_call_keeps_part_control_and_omits_all_position_arguments(
    ...
    eval(compile(ast.Expression(calls[0]), module.__file__, "eval"), scope)
...
def test_fresh_face_manifest_reresolves_without_retaining_old_handles(target, label):
    ...
    module = NS(
        _shank_face=resolver, _visible_cylindrical_face=resolver, JOURNAL_DIA=11.388
    )
    ...
    args = (
        ...
        else (adapter, view, module.JOURNAL_DIA)
    )
