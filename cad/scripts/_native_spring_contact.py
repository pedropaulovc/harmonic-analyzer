"""Seat unchanged supplier solids using the actual assembly interference gate.
...
def _cleanup_failure(primary: BaseException | None, cleanup: BaseException, context: str) -> None:
...
class _ActualContact:
    """Restored actual placements, never copied-body or speculative geometry."""

    def __init__(
        self, adapter: Any, moving: str, fixed: str,
        direction: tuple[float, float, float], maximum: float, label: str,
    ) -> None:
        ...
        self.names = [str(_read_member(component, "Name2")) for component in self.components]
        if moving not in self.names or fixed not in self.names:
            raise RuntimeError(f"{label}: native contact requires existing top-level components")
        component = _early_bound(self.asm.GetComponentByName(moving), "IComponent2")
        if not component.IsFixed():
            raise RuntimeError(f"{label}: native contact may only move a grounded component ({moving})")
    ...
    def _transform(self, name: str) -> list[float]:
        ...
        if len(data) != 16 or not all(math.isfinite(value) for value in data) or data[12] <= 0.0:
    ...
    def _assert_pose(self, name: str, expected: list[float]) -> None:
        ...
        if deviation > _TRANSFORM_READBACK_TOLERANCE:
            raise RuntimeError(f"{self.label}: {name} transform readback differs by {deviation:.17g}")
    ...
    def _put(self, target: list[float]) -> None:
        ...
        waypoint = max(
            self.endpoints,
            key=lambda values: sum((values[9 + i] - target[9 + i]) ** 2 for i in range(3)),
        )
    ...
    def _distance_mm(self) -> float:
        ...
        if not isinstance(result, tuple) or len(result) != 3:
            raise RuntimeError(f"{self.label}: unexpected ClosestDistance result: {result!r}")
        distance_mm = float(result[0]) * 1000.0
        if not math.isfinite(distance_mm) or distance_mm < 0.0:
            raise RuntimeError(f"{self.label}: native ClosestDistance has no finite solution")
        return distance_mm

    def _interference_state(self, expected: dict[str, list[float]]) -> Literal["interfering", "clear"]:
        ...
        try:
            ...
            for raw in manager.GetInterferences() or []:
                ...
                names = [str(_read_member(component, "Name2")) for component in _read_member(hit, "Components") or []]
            ...
            try:
                ...
                _cleanup_failure(primary, cleanup, f"{self.label}: interference Done failed")
    ...
    def _update_mates(self) -> None:
        ...
        extension = _early_bound(_read_member(self.model, "Extension"), "IModelDocExtension")
        if not extension.Rebuild(4):
            raise RuntimeError(f"{self.label}: native contact mate update failed")

    def evaluate(
        self, offset_mm: float, *, measurement: Literal["interference", "contact"] = "interference",
    ) -> tuple[Literal["interfering", "clear"], float | None]:
        if measurement not in ("interference", "contact"):
            raise ValueError("native contact measurement must be interference or contact")
        ...
        try:
            ...
            try:
                ...
                _cleanup_failure(primary, cleanup, f"{self.label}: original placement restoration failed")
            else:
                if primary is None:
                    _telemetry.debug(
                        f"{self.label}: native trial {self.trials} restored",
                        trial=self.trials, offset_mm=offset_mm,
                    )
...
def solve_component_contact(
    ...
    *,
    ...
    Actual component placement, rebuild, assembly interference, and minimum
    distance are used; every trial restores the original assembly pose.
    ...
    """
    if len(direction_xyz) != 3 or not all(math.isfinite(value) for value in direction_xyz):
    ...
    if not math.isfinite(max_translation_mm) or max_translation_mm <= 0.0 or not math.isfinite(2.0 * max_translation_mm):
    ...
    with _telemetry.span("spring.native_contact", label=label) as span:
        ...
        span.set_attribute("native_distance_guard_mm", NATIVE_CONTACT_DISTANCE_TOLERANCE_MM)
        try:
            pair = _ActualContact(adapter, moving_component, fixed_component, direction_xyz, max_translation_mm, label)
            ...
            if seed_state == "clear":
                if seed_distance <= NATIVE_CONTACT_DISTANCE_TOLERANCE_MM:
                    ...
                    return ContactSolution(0.0, None, 0.0, 0, "already_seated", "native_distance", seed_distance)
                if max_translation_mm >= _POSITION_CONVERGENCE_MM:
                    inward_state, inward_distance = pair.evaluate(-_POSITION_CONVERGENCE_MM, measurement="contact")
                    if inward_state == "interfering":
                        assert inward_distance is not None
                        span.set_attribute("certificate", "already_seated")
                        span.set_attribute("witness", "native_collision_bracket")
                        span.set_attribute("interfering_offset_mm", -_POSITION_CONVERGENCE_MM)
                        span.set_attribute("clear_offset_mm", 0.0)
                        span.set_attribute("bracket_width_mm", _POSITION_CONVERGENCE_MM)
                        span.set_attribute("interfering_endpoint_distance_mm", inward_distance)
                        span.set_attribute("final_distance_mm", seed_distance)
                        _warn_distance_disagreement(label, seed_distance)
                        return ContactSolution(0.0, -_POSITION_CONVERGENCE_MM, 0.0, 0, "already_seated", "native_collision_bracket", seed_distance)
            lo, hi = -max_translation_mm, max_translation_mm
            if pair.evaluate(lo)[0] != "interfering":
                raise RuntimeError(f"{label}: negative native bracket is not interfering ({lo:.17g} mm)")
            if pair.evaluate(hi)[0] != "clear":
                raise RuntimeError(f"{label}: positive native bracket is not clear ({hi:.17g} mm)")
            while hi - lo > _POSITION_CONVERGENCE_MM:
                if iterations >= _MAX_ITERATIONS:
                    raise RuntimeError(f"{label}: native contact bisection exhausted {_MAX_ITERATIONS} iterations")
                mid = lo + (hi - lo) / 2.0
                if not math.isfinite(mid) or not lo < mid < hi:
                    raise RuntimeError(f"{label}: native contact bracket cannot converge")
            ...
            return ContactSolution(hi, lo, hi, iterations, "bracketed_native_contact", "native_collision_bracket", clear_distance)
        finally:
            span.set_attribute("iterations", iterations)
