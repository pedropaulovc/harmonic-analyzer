...
async def build(adapter) -> dict[str, str]:
    ...
    # direction (the final horizontal top edge and the vertical closure are
    # supplied by closure) -- L0 shaft radius (H), L1 front length (V), L2 seat
    # step down (H), L3 seat length (V), L4 seat step up (H), L5 rear length
    # (V), L6 knob step (H), L7 knob length (V). Steps are radius DIFFERENCES,
    # driven as derived exprs.
    await define_rectilinear_chain(
        adapter, profile_lines, profile_pts, label="shaft", dims=profile,
        names=[
            "ShaftRadius", "FrontLength", "SeatStepDown", "SeatLength",
            "SeatStepUp", "RearLength", "KnobStep", "KnobLength",
        ],
        drives=[
            '"ShaftDia" / 2',
            '"FrontLen"',
    ...
    expected = math.pi * (
        (SHAFT_DIA / 2.0) ** 2 * (FRONT_LEN + REAR_LEN)
