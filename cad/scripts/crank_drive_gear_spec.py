...
# and the helix setting produce them, and the print's acceptance sizes are the
# three native dimensions above. The helix angle, its hand and the tooth
# thinning that opens the crossed mesh are the three facts a machinist cannot
# read off any view of this gear.
GEAR_DATA = gear_data_note(
    [
        ("NUMBER OF TEETH", f"{TEETH}"),
        ("DIAMETRAL PITCH, TRANSVERSE", f"{DIAMETRAL_PITCH:.2f} (NONSTANDARD)"),
        ("MODULE, TRANSVERSE (mm, REF)", f"{MODULE_MM:.3f}"),
        ("PRESSURE ANGLE, TRANSVERSE", f"{PRESSURE_ANGLE_DEG:.1f} DEG"),
        ("PITCH DIAMETER (mm, REF)", f"{PITCH_DIA:.2f}"),
        ("ROOT DIAMETER (mm, REF)", f"{ROOT_DIA:.2f}"),
        ("WHOLE DEPTH (mm, REF)", f"{WHOLE_DEPTH:.2f}"),
        ("HELIX ANGLE AT PITCH DIAMETER", f"{HELIX_ANGLE_DEG:.1f} DEG {HELIX_HAND}"),
        (
            "CIRCULAR TOOTH THICKNESS, TRANSVERSE (mm, REF)",
            f"{TRANSVERSE_CIRCULAR_TOOTH_THICKNESS:.3f}",
        ),
        ("TOOTH THINNING FOR BACKLASH (mm, REF)", f"{BACKLASH_MM:.3f}"),
        ("TOOTH FORM", "HELICAL INVOLUTE, FULL DEPTH, ARC ROOT FLOOR"),
        ("MATES WITH", "CRANK PINION MHA-025, 16T STRAIGHT SPUR"),
    ]
)
