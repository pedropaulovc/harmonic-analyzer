...
not on the GD&T allowlist and nothing runs on it in service, so it carries no
...
def test_overall_length_is_the_axis_line_driving_dimension() -> None:
    ...
    # (165.00, controlling), driven by the RodLength global; the right dome
...
def test_notes_are_the_stock_licence_only() -> None:
    ...
    lines = notes.split("\n")
    assert len(lines) == 1
    assert notes == "Ø6.0 ROUND BAR STOCK; OD OK AS RECEIVED."  # Lipton's licence
...
def test_print_carries_no_gdt_finish_or_basic_dimensions() -> None:
    ...
    assert not hasattr(magnifying_lever_spec, "GEOMETRIC_TOLERANCES_MM")
    assert not hasattr(magnifying_lever_spec, "SURFACE_FINISHES")
