...
_FORMAT_FIELDS = (
    "TypeFaceName", "CharHeight", "CharHeightInPts", "WidthFactor",
    "CharSpacingFactor", "LineLength", "LineSpacing", "Escapement", "ObliqueAngle",
    "Bold", "Italic", "Underline", "Strikeout", "BackWards", "UpsideDown", "Vertical",
)
_GAP_M = 0.002
...
def _table_state(table: Any) -> _TableState:
    dimensions = tuple(int(getattr(table, name)) for name in (
        "RowCount", "ColumnCount", "TotalRowCount", "TotalColumnCount",
    ))
    if dimensions != (18, 4, 18, 4):
        raise RuntimeError(f"harmonic-base table must retain visible 18x4 cells: {dimensions}")
    ...
    if int(table.AnchorType) != 1 or table.Anchored:
        raise RuntimeError("harmonic-base table must remain unanchored, top-left origin")
    ...
    for row in range(18):
        for column in range(4):
            ...
            if text is None:
                raise RuntimeError(f"harmonic-base table cell ({row}, {column}) has no text")
            cells.append((row, column, str(text),
...
def _annotation(table):
    ...
    if (int(annotation.GetType()) != 14 or int(annotation.Visible) != 1
            or int(annotation.OwnerType) == 2 or not annotation.GetName()):
        raise RuntimeError("harmonic-base table needs a named visible native table annotation")
    return annotation
...
def _inventory(adapter, table, expected_annotation):
    ...
    while view is not None:
        ...
        for raw in view.GetAnnotations() or ():
            if raw is None:
                raise RuntimeError("harmonic-base native annotation inventory contains null")
...
def _sheet_bounds(adapter):
    ...
    if (len(properties) != 8 or not all(math.isfinite(v) for v in properties)
...
def _compact_table(adapter, table, annotation, drawable, title_block):
    ...
    _telemetry.info("harmonic-base native table before compaction",
                    table_state=json.dumps(asdict(before)), position=position)
    actual_heights = []
    for row, original in enumerate(before.heights):
        ...
        if not math.isfinite(actual) or actual <= 0 or actual > original:
            raise RuntimeError(f"harmonic-base row {row} rejected minimum height: {actual}")
        readback = float(table.GetRowHeight(row))
        if readback != actual:
            raise RuntimeError(f"harmonic-base row {row} height readback differs: {actual}, {readback}")
    ...
    if _table_state(table) != expected or _position(annotation) != position:
        raise RuntimeError("harmonic-base compaction changed table content/font/width/padding/anchor")
    target = (drawable.xmax - sum(expected.widths) - _HEADROOM_M,
              drawable.ymax - _HEADROOM_M, position[2])
    bounds = _box(expected, target)
    _telemetry.info("harmonic-base native table minimum-height readback",
                    row_heights=expected.heights, target=target, bounds=bounds.bounds,
                    drawable=drawable.bounds)
    if (bounds.xmin < drawable.xmin + _HEADROOM_M
            or bounds.ymin < drawable.ymin + _HEADROOM_M
            or (bounds.xmax > title_block.xmin - _GAP_M
                and bounds.ymin < title_block.ymax + _GAP_M)):
        raise RuntimeError(f"harmonic-base compacted table no_fit: {bounds.bounds}, drawable={drawable.bounds}")
    ...
    if math.dist(_position(annotation), target) > _POSITION_TOLERANCE_M:
        raise RuntimeError("harmonic-base table did not reach measured upper-right position")
    if _table_state(table) != expected:
        raise RuntimeError("harmonic-base table placement changed content/font/row geometry")
    return expected


@_telemetry.traced("drawing.harmonic_base.native_layout")
def repair_harmonic_base_layout(
    adapter: Any, *, table: Any, views: dict[str, Any],
    manufacturing_note: Any, side_note: Any,
):
    ...
    def measure(actual_adapter, actual_annotation):
        ...
        if (int(actual_adapter.swApp.IsSame(annotation, actual_annotation)) != 1
                or int(actual_adapter.swApp.IsSame(annotation, _annotation(table))) != 1):
            raise RuntimeError("harmonic-base measurement encountered a replaced/unknown table")
        state = _table_state(table)
        if state != expected:
            raise RuntimeError("harmonic-base table content/font/row geometry changed during packing")
        ...
        return AnnotationBounds(
            name=str(actual_annotation.GetName()), kind=14, anchor=position[:2],
            body=bounds, envelope=bounds, text_boxes=(bounds,), text_runs=(),
            leader_segments=(), format_signature=(state,), native_strokes=(),
        )

    notes = (
        LayoutNote("manufacturing", _early_bound(manufacturing_note, "INote").GetAnnotation()),
        LayoutNote("side-caption", _early_bound(side_note, "INote").GetAnnotation(), follows_view="side"),
    )
    report = repair_native_layout(
        adapter, views=views, title_block=title_block, measure_annotation=measure,
        notes=notes, gap_m=_GAP_M, planning_headroom_m=_HEADROOM_M,
    ...
    _telemetry.info("harmonic-base native print layout measured",
    ...
    if report.status in (NativeLayoutStatus.NO_FIT, NativeLayoutStatus.SEARCH_LIMIT):
        raise RuntimeError(f"harmonic-base native print {report.status.value}: {report.reason}")
    return report
