...
DRAWING_NOTES = "\n".join(
    (
        f"{THREAD_DESIGNATION} FULL THREAD OVER {SHANK_LEN:g} LENGTH BELOW HEAD; "
        "THREAD FORM, RUNOUT, AND LIMITS PER ASME B1.1.",
        "THREAD GEOMETRY OMITTED IN VIEWS; CYLINDRICAL SHANK OUTLINE IS "
        "REFERENCE ONLY.",
        f"HEAD Ø{HEAD_DIA:g} X {HEAD_LENGTH:g} LONG; {GROOVE_COUNT} EQUALLY "
        "SPACED AXIAL Ø1.0 CYLINDRICAL GROOVES, CUTTER AXIS ON HEAD OD, FULL "
        "HEAD LENGTH.",
    )
)
