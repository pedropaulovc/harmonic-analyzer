...
slotted fillister) through its #19 foot holes into bottoming-tapped seats in
the support's top rail, transferred from the set bracket at assembly (the
south bracket is feeler-set, so its seats cannot be cast or pre-drilled).
...
# the foot face, so the window is symmetric about local x only.
RAIL_DEPTH = 21.0  # .X on the print
WINDOW_TOP_Y = HALF_Y - RAIL_DEPTH  # local 67.9 (machine y 207.6)
WINDOW_BOTTOM_Y = FOOT_THICKNESS - HALF_Y  # local -82.55, unchanged
WINDOW_HEIGHT = WINDOW_TOP_Y - WINDOW_BOTTOM_Y  # 150.45
...
if SEAT_DRILL_BOTTOM_MAX + EDGE_BREAK > RAIL_DEPTH - LINEAR_1PL:
    raise AssertionError(
        f"bracket seat drill point reaches {SEAT_DRILL_BOTTOM_MAX:.2f} deep, through"
        f" the {RAIL_DEPTH:.1f} (.X) rail into the window"
    )
