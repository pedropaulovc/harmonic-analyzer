...
run, then they hold:
    ...
    p2  pinion engage   -- the bracket+alignment-pinion group swings on the torque
...
async def _drive_p2(adapter: Any) -> dict[str, str]:
    """p2: the bracket+pinion rigid group swings on the torque shaft to engage.
    Free the swing (the alignment-pinion's lone spin park driver), then motor a
    bracket about its pivot bore (Axis1, collinear with the torque shaft)."""
    path = str(OUT_SLDASM / "drive-train.SLDASM")
    check("open drive-train", await adapter.open_model(path))
    await _suppress(adapter, _family_driver_names(
        adapter, "drive-train", "alignment-pinion"), "p2 pinion swing park")
