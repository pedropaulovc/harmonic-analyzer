...
def test_drawing_places_bom_and_balloons() -> None:
    ...
    assert "position_xy=(0.150, 0.066)" in source
    assert 'item_number="2"' in source
    assert "position_xy=(0.255, 0.105)" in source
...
def test_manual_balloon_moves_are_locked_and_read_back() -> None:
    ...
    assert helper.count("current_annotation = note.GetAnnotation()") == 2
