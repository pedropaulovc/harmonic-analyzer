...
DRAWING_NOTES = "\n".join(
    (
        ...
        "  77.62+/-1.00 DEG CCW FROM DATUM EDGE B IN FRONT PROFILE.",
        "FREE-FLAT AXIS 97.62+/-1.00 DEG CCW FROM +B RAY IN FRONT PROFILE;",
        "  +B RUNS FROM LEFT FREE END TOWARD BEND; FREE FLAT IS NEAR VERTICAL.",
        "HOLE <MOD-DIAM>3.25+/-0.05 THRU; AXIS 3.10+/-0.10 FROM LEFT FREE END",
        "  AND 2.00+/-0.05 FROM DATUM EDGE B IN TOP VIEW.",
        "DATUM A IS THE FOOT-SIDE BROAD FACE (NORMAL TO TOP VIEW).",
        "FORMED BROAD FACE IS THE BLADE-SIDE BROAD FACE (NORMAL TO FRONT VIEW).",
    )
)
