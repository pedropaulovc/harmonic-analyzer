...
DRAWING_NOTES = "\n".join(
    (
        ...
        "FREE-FLAT AXIS 97.62+/-1.00 DEG CCW FROM DATUM EDGE B IN FRONT PROFILE.",
        "HOLE <MOD-DIAM>3.25+/-0.05; AXIS 3.10+/-0.10 FROM THE LEFT FREE END",
        "  AND 2.00+/-0.05 FROM DATUM EDGE B IN THE REMOVED TOP VIEW.",
        "DATUM A IS THE BROAD FOOT FACE SHOWN IN TOP VIEW; NO TWIST.",
    )
)
