...
| Cylinder arbor (stationary) | x = −47.5 (frame-locked: SOUTH arbor pedestal only — the rocker boss bore is gone with the portal unification; M6.12 shortened the arbor 200 → 176, north end z +78 unsupported pending the cone rework), y = 76, along Z | the cone grid is derived FROM the drum (X_PITCH + exact tracking); the book's "oblique partial engagement, distinct wear" (ch. 12) is now modeled literally (M6.7). Vertical connecting rods reach rocker tips only for x_cyl ≥ −100 ✓ | med |
...
| `arbor-pedestal` ×1 (M6.5, M6.9 trim, M6.12 south-only) | green block 24 × 16 × 85, Ø9.5 clamp bore along Z | M6.12: SOUTH end only, at (−47.5, −90.5). The rocker support no longer clamps the arbor (its boss/bore went with the portal unification), but the now-solid portal north upright leaves no room for a north pedestal where the arbor's north end was — so the arbor is SHORTENED (200 → 176, spans z −98..+78) to clear the frustum south face (~+85.6 at the drive axis), and its north end is left unsupported. The dedicated north-end support is DEFERRED to the cone-position rework. South block front face −98.5 clears the portal south-plate back face −99 by 0.5 (M6.9); stack ends ±70.6 | function-driven; v3 side view (6.124 px/mm) | low/med |
...
| cone small-end support — DEFERRED (M6.12) | — | M6.12: the cone small end should get its own bracket (the role the old Ø32 `cone-knob-post` filled and the rocker boss wrongly absorbed), but the cone is currently mis-positioned and the whole north drive region will be re-laid out. A bracket placed against the wrong cone axis only collides with the solid portal, so the `cone-knob-post` is retired and the cone tip is left UNSUPPORTED until the cone-position rework, when the bracket is designed and placed properly | p.18 tip-rest role | deferred |
...
| Arm length | symmetric ±114.3 mm (4.5") about the pivot (228.6 = 9" total) | 4.5 | REDERIVED from the p.29 broadside photo: pivot ball, rod-pin hole and the 16 mm depth callout give a dual scale (~8.1–8.6 px/mm), and the pivot→tip span measures 113–119 mm (central ~116) → adopt the clean imperial 4.5" half (9" total = the 9" pivot shaft). Supersedes the indirect M6.3 "80 mm bar travel + 8 mm margin" = 88 (never measured) and the M2 100/70 asymmetric read; ch. 15 symmetry retained | photo-scaled | med |
...
| Support casting ×1 (M6.5, depth re-read M6.9, UNIFIED M6.12) | M6.12: the windowed portal that M6.9 confirmed is now ONE green casting, `rocker-arm-portal` (build_rocker_arm_portal.py), replacing the former two-part split (this frustum + the transgear A-frame). NORTH upright = solid tapered frustum, base 88.9 × 40 → top 20 × 20, 177.8 tall (7"); SOUTH upright at machine z −111 = the tapered plate + saddle + clevis ears (former a-frame row); the two are tied by the TOP and FOOT rails, which now run continuously between them (≈2 mm overlap into each upright fuses the casting into a single solid body — gated in the build). The old east-flank arbor-clamp boss Ø20 / Ø9.7 bore is GONE: the cone/arbor no longer rests on the support (the arbor's north end gets its own pedestal, the cone tip its own bracket, both in drive-train.SLDASM). Authored machine-handed → inserted unmirrored in frame.SLDASM at (x, z) = (+72.9, +101.6), the same transform the frustum used. | front view: triangle base spans x −130…−44 (≈86), apex at −81 ± 6. M6.9 REINSTATED the windowed-frame reading M6.3/M6.5 refuted: ch30 p008 (+x side, brightened) plainly shows the windowed portal frame (~184 wide, ~127 window). The p008 uprights read ~28–40 deep and near-uniform → side depth 40 → 20 (supersedes the legacy 63.5 → 16.9 strong taper). M6.5's refutation of a second free-standing SOUTH frustum stands (the calibrated v3 side view shows ONE frustum at the back, z +69..+134 footprint; the south ball is gripped by the clevis). M6.12: photo Z-scale is perspective-compressed, so the casting's Z geometry stays anchored on the mechanically-locked neighbours (both pivot balls, the channel grid, the parked stick z ≤ −118, the ch25 handle z ≥ −98). | photo + legacy height | med |
...
| Hold-down lag screws ×2 (M6.10) | round head Ø14 × 4 + Ø7.8 × 66 shank, axis +Y at machine (x, z) = (72.9 ± 31.75, 101.6) — UP through the base from below: heads recessed in the underside's Ø15 × 4.5 counterbores (y 0.5..4.5), shanks through the base's Ø8.2 through-holes into the frustum's Ø7.94 × 25 sockets (tips y 70.5, 5.3 short of the socket bottoms) | the support's 63.5 mounting-hole pitch (`build_rocker_arm_portal.py`); placed in frame.SLDASM (`build_lag_screw.py`) | low |
...
| Top frame (NEW part) | green cast rectangular ring around the 4 columns: rails 22 wide × 41 tall (y 999.7…1040.7, top = lever ball-mount seat 1065.9 − 25.2), corner bosses Ø48 bored Ø25.5 (column Ø25.4, OD rederived from the 8-views M6.11) at (±197, ±112); clamps the columns 80 below their tops (1120.8 − 1040.7) | eight-views: green ring at y ≈ 1010–1055 in all views, columns continue above it to caps ≈ 1090–1120 | med |
| Ball mounts ×4 | rocker pair at seats y 228.6: north on the support apex (z +101.6), south on the A-frame saddle (z −111, M6.5); lever pair on the top-frame west rail (seat y 1040.7) at z ±85 — the Ø16 base must clear the Ø25.5 boss bores (need √(2.9² + dz²) ≥ 12.75 + 8 → dz ≥ 20.5 from the boss at z 112; z 85 gives dz 27, comfortably clear after the OD rederivation M6.11 loosened the old dz ≥ 25.3) | ch. 14 / ch. 17; lever z from boss-bore clearance | med |
...
| Gooseneck | Ø16 chrome tube: vertical leg machine x 197 (east column line), y 880..1390 — the post drops ≈120 below the rail underside (999.7) and ends in a free tip, passing through a Ø17 clearance bore in the east rail (`build_top_frame`) and gripped by the clamp above; 180° bend R 51; tip leg x 95 — plumb above the boss hook — ending y 1378; lug + Ø4 X-pin at y 1373 carries the spring's top loop | — | tip drop: ch. 7 back-left ¾ view (page001_img17) — free tip clear of the columns at image y 491, machine 880 by frame-height scale (41 mm/26 px), cross-checked by the Ø16 silhouette (10 px). Leg-top/bend: 0.515 px/mm + p3 90° page; tip "slotted screw" modeled as lug + pin | scaled + derived | leg-top med, drop med |
...
clearance bore and drops ~120 below the plate underside, ending in a free
tip at machine y 880 (below the rail bottom 999.7) — the support extends
well below the top plate (ch. 7 page001_img17 back-left ¾ view).
...
| Pinion bar (M6.5) | 12-square steel, x −58 → +178 at machine y 253.5 (M6.5 trim from −95..+197: west end clears the portal south-upright clevis ears/ball at x −59, east end clears the SE column tangent 184.3 (was 179.54 at the old Ø34.925; OD rederived from the 8-views M6.11 — 6.3 clearance now)); Ø9.6 stud bore along Z at x 0 (9.525 stud + slip clearance); both ends FLOAT (documented simplification — the real machine likely straps the bar to the portal south upright and column) | — | ch. 30 views + M6.5 layout | scaled + derived | low/med |
| A-frame → folded into `rocker-arm-portal` (M6.5/M6.9, UNIFIED M6.12) | M6.12: no longer a separate part — these are the SOUTH-upright + rail features of the single `rocker-arm-portal` casting (build_rocker_arm_portal.py), re-centred on machine x +72.9 and authored machine-handed in the frame assembly's frame. The two south-flank hold-down hex bolts move to frame.SLDASM with the foot rail. Geometry below unchanged. The FRONT rocker support — the SOUTH upright of the portal frame (ch30 p008): tapered plate 18.5 thick (machine z −117.5..−99.0 — photo reads ~28-30 but the band is pinned by the parked measuring stick at z ≤ −118 and the ch25 handle plane at z ≥ −98), foot x −115..−45 on the base top (y 50.8), apex x −87..−59 at the ball-seat saddle y 228.6 (machine); full-width saddle z ±11.1 over the last 19.8 of rise; clevis ears (z ±(8.1..11.1), 20 tall, tops machine 248.6) flank the south pivot-ball mount's Ø16 base (gap 16.2; ball r 9.5 centre 253.8 clears the ear inner faces; pivot shaft bottom 250.65 clears ear tops by 2.05); carries the portal-frame rails to the north frustum (faces −0.25): TOP RAIL 20 (machine x +62.9..+82.9) × 16 (y machine 212.6..228.6 = photo window top) to machine z +90.45, FOOT RAIL 30 (machine x +59.75..+89.75 — west face 0.25 east of the arbor-pedestal block) × 20 on the base top to machine z +81.35, bolted down by two hex bolts (M6.10): AF 12.7 × 5.5 heads + Ø7.8 × 32 shanks at machine (74.75, z −54/+36) through the rail's Ø8.2 holes, 12 into the base's through-holes (head corner 2.64 clear of the cylinder train's 120T tips); M6.12: the part is authored MACHINE-handed with NO MIRROR_PLANE entry (it lives in the non-mirroring frame.SLDASM) — the real machine casts both uprights + rails in one piece, now modeled that way | — | ch. 14 pp. 26–29 + ch. 30 front view: clevis grips the pivot ball at (−72, 252) — M6.5 apex crop; ch30 p008 (brightened) shows the full windowed frame; the M6.4 "ears grip the pinion bar" read is REFUTED (bar now floats) | scaled | med |
...
| rocker-arm-support ×3 → `rocker-arm-portal` ×1 (M6.12) | none | M6.3 refuted the legacy windowed square frame (184 wide); M6.9 PARTIALLY REINSTATED it — ch30 p008 (brightened) shows exactly that frame: north frustum + south upright + top/foot rails (ch. 14 layout table). The ×3 count stays refuted (ONE frame: two uprights; the legacy third instance at x ≈ 0 under the transgear is the pinion-bar/stud's job). M6.12: the two-part split (rocker-arm-support frustum + a-frame upright) is unified into the single `rocker-arm-portal` casting |
...
4. **Rocker arm working length** — RE-RESOLVED in M6.3, then REDERIVED
   from the p.29 broadside photo (supersedes the indirect ±88 and the
   M2 100/70 model): symmetric ±114.3 (4.5") about the pivot, rod pin at
   +25.4 — see the ch. 14 "Rocker pivot & supports layout" table. The M2 read
