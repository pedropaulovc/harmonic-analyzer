...
- Under `--executor farm` each leaf gets 15 min on the worker by default. A cold
  ...
  clamps the request to 60 s–4 h. Leaving it at the default on a cold run costs
...
- **Telemetry must not cost seat time.** Two measured traps, both fixed, both worth
  ...
  protocol and pins each reachable signal endpoint together with its protocol.
  ...
  (`run_build`'s tail and the watchdog before `os._exit`). Timeout environment
  values use the standard integer-millisecond unit. Signal-specific
  `OTEL_EXPORTER_OTLP_TRACES_TIMEOUT` / `OTEL_EXPORTER_OTLP_LOGS_TIMEOUT` override
  global `OTEL_EXPORTER_OTLP_TIMEOUT`. Empty values fall through; malformed,
  non-positive, or oversized values warn and fall through. If neither setting
  supplies a positive 32-bit millisecond value, each OTel Python export call gets
