...
how would you manufacture the cone gear set? remember that the entire operation of the analyzer depends on it so we must be precise. Consider the two options below when providing your answer

# Option 1
Scope & assumptions
item	choice	rationale
Gear standard	32 DP, 20 ° PA, full-depth involute	Gives a 0.25 in OD for the 6-tooth gear and 3.81 in for the 120-tooth gear—both well inside the 7 × 28 in table and 19 × 10 × 16 in travels of your PM-728VT
. 32 DP is also close to the ≈0.8 mm module suggested by the original machine (6-tooth gear ≈6 mm OD).
Pressure-angle variant	Standard full-depth for ≥ 12 teeth; 0.010 in positive profile shift (or a stub cutter) for the 6-tooth gear	Prevents under-cut and preserves root strength.
Tooth face width	0.20 in on all 20 gears	Wide enough for load, still leaves the cone < 4.5 in long.
Material	C360 brass for gears 5–20; 510 phosphor-bronze for gears 1–4 (6–24 T)	Matches the “slightly more yellow” wear-resistant tip gears noted in the book
.
Shaft	Ø ½ in 12L14 steel, turned with 20 shoulders stepped 0.23 in	Shoulders give the cone profile and a positive axial datum for each gear.

    Resulting envelope: cone diameter steps from 0.25 in to 3.81 in in 0.19 in increments; overall cone length ≈ 4.60 in plus a ½ in journal at each end.

1 CAD preparation (Fusion 360 personal-use)

    Spreadsheet parameters

    DP = 32        // diametral pitch
    PA = 20 deg    // pressure angle
    N[1..20] = 6,12,…,120

    Gear script → generate 20 involute spur gears (Design ▸ Tools ▸ Spur Gear).
    For the 6-tooth gear set Addendum Offset = +0.010 in.

    Cone assembly

        Place gears concentrically on the shaft shoulders.

        Add 0.020 in brass shims between gears to guarantee clearance.

Export each body as STEP for CAM; Fusion personal use can post 2- and 3-axis tool-paths, which is all we need.
2 Machining workflow on the PM-728VT

    You will need: 5⁄8-in stub arbor, 32 DP involute cutter set (#12 for 6 T, #7 for 12 T, …, #1 for ≥ 55 T), a 40∶1 dividing head & tail-stock, and a right-angle milling bracket.

step	operation	tips
2.1	Turn blanks on the lathe	OD = (N + 2)/DP. Face to 0.20 in width. Bore 0.501 in ream for a 0.0005 in press fit on the ½-in shaft.
2.2	Cut teeth on mill	Clamp the arbor between dividing head & tail-stock, cutter on the spindle. Set cutter centerline at gear mid-height. Use table: index = 40 / N turns. Feed 1–2 ipm at 250–350 SFM (≈ 400 RPM in brass).
2.3	Deburr	Small India stone on both flanks.
2.4	Hard-brush phosphor-bronze gears	Slight work-hardening improves wear.

Optionally, outsource hobbing—the diameter range is small enough for commercial 32 DP hobs, and the cost is often lower than buying the eight involute cutters you’ll need.
3 Shaft & assembly

    Shaft turning

        Turn Ø 0.5005 in journal at each end, Ø 0.5000 in shoulders for gears, shoulders spaced 0.23 in (0.20 in gear + 0.02 in spacer + 0.01 in clearance).

        Cut a 1⁄8 in keyway full length only if you later decide to key the gears; the originals relied on press fit + solder peening.

    Press fit sequence

        Freeze the shaft (−10 °F freezer).

        Heat each gear to 250 °F on an electric hot plate.

        Slide onto its shoulder against a stop collar; peen a 1⁄16 in dimple at the joint and wick in Loctite 609.

        Work from the large-diameter end toward the tip so you always have a surface to tap against.

    Axial preload – secure the stack between two 15 mm bronze thrust washers and a 4-40 axial screw into the shaft end.

4 Quality checks
check	target	method
Composite TIR (gear rim)	≤ 0.001 in	Dial indicator while rotating on V-blocks.
Pitch error	≤ 0.0005 in cumulative	Roll meshing check against a hardened 32 DP master gear.
Concentricity (gear ➜ shaft)	≤ 0.0003 in	0.001 in DTI on tooth flanks.
Stack axial run-out	≤ 0.002 in over full cone	Surface plate + height gauge.
5 Risks & mitigations
risk	mitigation
Undercut in 6-tooth gear	Positive profile shift or stub cutter; if you outsource, specify 12½° stub, 32 DP.
Cutter breakage in brass	Use flood coolant or mist to prevent chip welding.
Thermal growth when pressing gears	Keep shaft cold and limit soak time to < 2 min per gear.
Cumulative indexing error	Always rough-cut first tooth, check full rotation, adjust before final depth pass.
Why this approach works

The original analyzer’s cone set merely supplies twenty precise gear ratios in a compact package; accuracy in concentricity and pitch matters far more than absolute strength. Using 32 DP brass gears keeps cutting forces and tool costs low while reproducing the 6 → 120 T progression Michelson specified
. All operations fit comfortably inside the spindle speed, horsepower, and travels of your PM-728VT mill

, and every tolerance is achievable with standard shop practice and minimal fixturing.

Once the cone is finished you can tackle the mating cylinder gear set—all identical 32 DP, 48 T gears—which is straightforward by comparison.

# Option 2
Objective

Produce a 20-gear “cone” that gives the exact linear tooth-count progression
6-12-18 … 120 and therefore the correct harmonic frequency ratios 1 : 2 : 3 … 20.
Because the analyzer runs essentially unloaded, our target is AGMA quality 10 (≈±25 µm composite error) – tight enough for phase fidelity but still feasible on a benchtop mill.
1. Design parameters (derived from the original)
Symbol	Value	Rationale
N1	6 teeth	fundamental gear count in book
ΔN	6 teeth	constant increment, keeps frequency ratios integer
Nmax	120 teeth	6 + 19·6
Pitch system	Module 1.5 mm (equiv. ≈16.93 DP)	Gives pitch Ø of 9 mm for N = 6 and 180 mm for N = 120 – almost identical to photographs
Pressure angle	20° full-depth involute	Easier cutter availability than 14½° and fine for light loads
Face width b	4 mm for N≤30, 6 mm for 36–60, 8 mm for 66–120	Matches visual aspect ratio and gives uniform tooth stiffness
Materials	- N ≤ 24: C932 (SAE 660) bearing bronze → galling-resistant, sacrificial
- N ≥ 30: 1144 stress-proof steel, case-hardened 0.4 mm	Mirrors colour / wear pattern noted in book and gives wear gradient
Bore	10 mm reamed + single 2 mm × 2 mm keyway down the common shaft	
Axial spacing	6 mm (gear) + 0.8 mm phosphor-bronze washer → 6.8 mm pitch	Leaves 0.2 mm running clearance against the mating cylinder gears

Assumptions you may want to confirm: actual cone stack length (photograph + calliper on museum unit) and exact pressure angle; either can be adjusted before CAM without affecting the integer tooth ratios.
2. Manufacturing route
2.1 Gear cutting strategy
Tooth count	Problem	Method
N ≤ 12 (6 & 12)	Standard involute cutters undercut below 12 T	CNC single-point generation (3 + 1 axes) using a rotary table and a 2 mm ball end-mill. A Fusion 360 “circular-pattern – separate toolpath per tooth” template works well.
N = 18 – 120	Within range of module-1.5 HSS form cutters	Manual form cutters on an indexing head. Use the standard 8-cutter set; mapping (N, cutter #) = (18-23→#5, 24-28→#4, …, 54-134→#2, 135-rack→#1).

Why not hobbing? You could, but synchronising table feed on the PM-728VT is non-trivial and the single-point/COTS-cutter approach is faster for a one-off set.
Workflow (per gear)
    ...
    Turn blank
        ...
        Face width as table above

        Bore 9.9 mm, leave 0.5 mm chamfer both sides.

        Drill 2 mm witness hole on pitch circle – used later for broaching alignment.

    Mount on arbor (mandrel turned from 1144, M6 draw-bolt) and cut teeth.
    For CNC pieces – program involute from the standard equation, run 0.1 mm finishing spring pass.
    For form-cutter pieces – 200 RPM, climb cut, flood coolant, 0.25 mm radial DOC per pass.

    Deburr & tumble in fine ceramic media 15 min; the small gears get a quick lapping on 1500-grit diamond paste to smooth the undercut.

    Heat-treat steel gears

        Carburise 860 °C 90 min, oil quench 60 °C, temper 180 °C 1 h → 58 HRC case.
        Bronze gears only need stress-relief 260 °C 1 h.

    Ream bore 10.000 ± 0.005 mm post-heat-treat; broach 2 mm keyway referenced to witness hole.

2.2 Shaft & assembly

    Shaft – 12 mm 1144, turn shoulders every 6.8 mm (see axial spacing), leaving 20 µm press-fit land for each gear.

    Fixture – simple stepped mandrel in tailstock so each gear seats against its shoulder as it is pressed on (arbor press).

    Key – continuous 2 × 2 mm square key, Loctite 638 along full length, then stake at both ends; this prevents relative slip without pinning every gear.

    End-caps – M6 × 1 nuts + 0.2 mm shim-washer set to pre-load stack axially but still allow cone to free-wheel; stake nut.

Run-out target after assembly: ≤ 15 µm TIR at largest gear pitch diameter – verified with dial indicator.
3. Metrology & verification
Step	Tool	Pass/fail criterion
Pitch error	Gear span micrometer on wires	±25 µm cumulative
Tooth profile	Optical comparator 20× overlay	chordal deviation < 15 µm
Bore-to-pitch concentricity	Dial indicator on bore while indexing	≤ 10 µm
Stack alignment	Surface plate + height gauge on one reference tooth per gear	±0.05 mm linear

Because torque is negligible, backlash ≤ 0.1 mm is acceptable; concentrate on concentricity and cumulative pitch.
4. Alternatives if you hit capability limits

    Outsource micro-pinions (6 & 12 T) to a gear-jobber with wire-EDM; cost is usually < US$60 each in 4140.

    Order whole set in laser-sintered bronze (e.g. Renishaw’s 420 SS/Bronze) and post-machine bores – dimensional accuracy can reach ±0.04 mm now; only finish grind the 6-T involute faces.

    3-D print + lost-wax cast for museum-grade authenticity; you still machine the functional surfaces.

5. Next actions

    Confirm module & pressure angle by measuring any original gear (photogrammetry works if you can’t access the artefact).

    Build parametric Fusion template with variables {N, module, pressure_angle, face_width}.

    Prototype the 18-tooth gear first – easiest to cut, lets you validate cutter choice and heat-treat schedule.

    Once satisfied, run the production batch and keep an inspection log; you will need those numbers when you tune the cylinder gear set later.

Recap

    Twenty spur gears, module 1.5 mm, 20° PA, teeth 6 → 120 in steps of six.

    Smallest two gears CNC-generated; the rest cut with standard M1.5 form cutters.

    Bronze for N≤24, case-hardened steel above that to mimic wear pattern and balance contact stress.

    Stack pressed and keyed on a precision 12 mm shaft; total run-out ≤ 15 µm.

Follow this sequence and you’ll have a cone gear set precise enough that the analyzer’s frequency ratios stay within ±0.03 %, well below the observable threshold in the plotted waveform.
...
```json
{
  ...
  "thoughts": [
    {
      "summary": "Refining gear manufacturing plans",
      "content": "The user wants help with the manufacturing process for a cone gear set, comparing two options they provided. I\u2019ll focus on selecting the better option or a hybrid approach, assuming the manufacturing process hasn\u2019t changed recently. I\u2019ll need to give clear, technical instructions on how I would handle the gear set, using precise language. I'll address any ambiguities as I review their options. If something\u2019s unclear, I\u2019ll ask for more details. We\u2019re good without external sources for this task."
    }
  ],
  "source_analysis_msg_id": "7b098867-b087-438b-9af3-63a2f9709be2"
}
```
...
**Cone gear set – manufacturing plan (selected route + fallback)**  

---

### 1  Option audit & decision
| Attribute | **Option 1** (32 DP, 20 ° PA, brass/bronze) | **Option 2** (m 1.5 mm, 20 ° PA, bronze/steel) | Comment |
|-----------|--------------------------------------------|--------------------------------------------------|---------|
| Pitch / cutters | U.S.–standard diametral-pitch; 8 form cutters #12 … #1 readily stocked | Metric module; will require M1.5 cutter set or custom ground hobs | Your shop & suppliers are imperial-oriented; 32 DP wins. |
| Min tooth count | 6 T handled via +0.010 in profile shift or 12 ½ ° stub cutter | 6 T cut by CNC single-point | Either works; profile-shifted form cutter is faster on a dividing head. |
| Face width | 0.20 in uniform | 4–8 mm graduated | 0.20 in gives consistent arbor shoulders and easier fixturing. |
| Materials | C360 gears 5–20, 510 PB gears 1–4 (matches “slightly more yellow” original) fileciteturn0file0 | Mixed bronze / case-hardened 1144 | C360 machines cleanly; PB where it matters for wear. |
| Shaft Ø | ½ in 12L14 | 12 mm 1144 | ½ in keeps all tooling imperial. |
| Tolerances targeted | ≤0.001 in composite TIR | ≤15 µm | Both within reach; imperial gauges cheaper. |

**Decision:** adopt **Option 1** wholesale; keep Option 2’s single-point fallback only for the 6-tooth gear if profile-shift proves marginal.

Assumptions (flag if wrong):  
* You can source a 40∶1 dividing head with tailstock.  
* You will press-fit, not solder, the gears.  
* Cone stack length target ≤ 4.75 in to clear analyser frame.

---

### 2  CAD prep (Fusion personal-use)
...
Parameters:
    ...
    N[]     = {6, 12, 18, …, 120}
    face    = 0.20 in
    add_off = +0.010 in   // only for N = 6
    shim    = 0.020 in
    step    = face + shim + 0.010 in clearance = 0.23 in
```

* Run **Design ▸ Tools ▸ Spur Gear** twenty times (scriptable).  
* Suppress keyway in models—press fit first, broach later only if slip occurs.  
* Build cone assembly on a virtual 0.5000 in shaft; check clash envelope.

Export **STEP** per body; personal licence supports 2- & 3-axis CAM needed.

---

### 3  Blank manufacture (lathe)

| Parameter | Formula | Example (18 T) |
|-----------|---------|----------------|
| OD | (N + 2)/DP | (18+2)/32 = 0.625 in |
| Bore | 0.501 in ream (for 0.0005 in press on 0.5005 shaft) | — |
| Width | 0.200 in ± 0.001 | — |
| Chamfer | 0.010 × 45 ° both faces | — |

*Keep a single soft-jaw chuck setup; turn all blanks before moving to milling.*

---

### 4  Tooth cutting on PM-728VT

1. **Setup**  
   * Stub arbor ⅝ in, gear blank clamped between needle-bearing washers.  
   * Cutter selection:  
     * #12 → 6 T (addendum +0.010 in)  
     * #7 → 12 T  
     * …  
     * #1 → 55 – 120 T  
   * Centerline: cutter mid-height = half face width.

2. **Indexing** (40∶1 worm)  
   `turns = 40 / N` (e.g., 40/18 = 2 + 4/9 turns).

3. **Feeds & speeds (C360 / PB)**  
   * 400 RPM (≈ 335 SFM)  
   * 0.002 ipm manual feed / light climb cut.  
   * Mist coolant; brush chips every pass.

4. **Depth of cut**  
   * Rough 0.010 in radial per pass; final spring pass 0.002 in.

5. **Deburr**  
   * Fine India stone on both flanks, then brass wire wheel.

*Fallback:* if 6-T undercut is unacceptable, swap to 3-axis CNC rotary, 2 mm ball EM, full 3 D involute tool-path; one-off only.
...
| Step | Spec |
|------|------|
| Material | 12L14, stress-relieved after turning |
| Geometry | Ø 0.5005 × 0.50 in journals, Ø 0.5000 × 0.23 in shoulders, total 5.6 in length |
| End prep | #4-40 tapped both ends + 15 mm PB thrust washers |

**Press sequence**

1. Shaft into freezer (-10 °F, 30 min).  
2. Pre-heat gear on hot-plate 250 °F.  
3. Slide from large (120 T) toward tip; seat against machined stop collar.  
4. Center-punch joint + wick Loctite 609.  
5. After full stack, install thrusts + 4-40 screw @ 5 lbf in preload.
...
| Check | Target | Method |
|-------|--------|--------|
| Rim run-out (any gear) | ≤ 0.001 in | V-blocks + DTI |
| Pitch cumulative | ≤ 0.0005 in over 120 T | Roll against 32 DP master |
| Bore-to-PD concentricity | ≤ 0.0003 in | Dial in bore, probe tooth |
| Stack axial error | ≤ 0.002 in | Surface plate, height-gauge |

Reject / re-cut any gear exceeding 2× limits.
...
| Hazard | Action |
|--------|--------|
| **Undercut 6 T** | Profile shift already; keep tool sharp, light DOC. |
| **Cutter galling in brass** | Continuous mist or flood; never cut dry. |
| **Press gall-up** | Strict 250 °F cap, assemble within 2 min each. |
| **Index error accumulation** | Trial-cut first tooth shallow; verify full rotation before final depth. |
| **Shaft bend while pressing** | Support free end on live centre, press axially. |

---

### 8  Open items (need your go/no-go)

1. Do you want a continuous ⅛ in keyway broached post-assembly?  
2. Preference for outsourcing hobbing (~US $22/gear) vs. purchasing eight involute cutters (~US $400 total)?  
3. Confirm that cone length 4.75 in clears the frame envelope in your CAD.

Reply with adjustments and I’ll update the CAM setup sheets.
...
yes for continuous ⅛ in keyway broached post-assembly; avoid freeze / hot plate if possible; I will try buying the involute cutters. provide a full list to buy
...
```json
{
  ...
  "thoughts": [
    {
      "summary": "Clarifying recommendations for involute cutters",
      "content": "The user wants a continuous \u215b in keyway broached post-assembly and prefers not to use freeze or hot plate methods. They seek an alternative assembly method, like using a hydraulic press with Loctite or cryogenic aerosol for press fit.\n\nAs for buying involute cutters, we previously recommended a set of 8 involute cutters (#12 to #1). The set covers 32 DP, 20\u00b0 PA, full-depth gears. I need to finalize the list with proper sizes for specific requirements, like covering tooth counts starting from 6 teeth."
    },
    {
      "summary": "Mapping cutter numbers for tooth counts",
      "content": "The 32 DP 20\u00b0 PA full-depth cutter set typically involves 8 cutters. Cutter #1 handles 135 teeth and above, #2 goes for 55-134, and so on. For 6 teeth, a stub cutter like #12 is typically used. However, the earlier plan involves #12 for 6 T, and I need to clarify the mapping. Normally, sets cover up to #8 cutters, but some extensions may go up to #12 for smaller tooth counts. Looks like I'll need to check for specific extended sets for this."
    },
    {
      "summary": "Confirming cutter specifics for 32 DP",
      "content": "Standard 20\u00b0 PA involute cutter sets cover from 12 to 135 teeth, with a set of 8 cutters. For tooth counts under 12, special stub cutters are needed. The user plans to use a full set (#1\u2013#8) plus a stub cutter (#12 for 6 teeth). Option 1 mentions using a positive profile shift or stub cutter for 6 teeth. I'll provide a list of cutters, including the stub cutter, and check recommended vendors like McMaster-Carr, KBC Tools, or Amazon for pricing and details."
    }
  ],
  "source_analysis_msg_id": "210c344d-05b6-414b-be87-ec6f9933f1ac"
}
```
...
```json
{
  ...
  "text": "{\n  \"search_query\": [\n    {\n      \"q\": \"32 DP involute gear cutter numbers and tooth range\",\n      \"recency\": null,\n      \"domains\": null\n    },\n    {\n      \"q\": \"32 pitch gear cutter #8 tooth range 20 degree full depth\",\n      \"recency\": null,\n      \"domains\": null\n    }\n  ],\n  \"response_length\": \"short\"\n}"
}
```
...
```json
{
  ...
  "thoughts": [
    {
      "summary": "Understanding cutter limitations",
      "content": "Standard 20\u00b0 full-depth involute cutter sets typically cover tooth counts from 12 to 135 teeth, using a set of 8 cutters. For fewer than 12 teeth, you need specialized cutters, often with a longer addendum or a stub profile. Cutters like #9-#14 are used for these smaller counts, but they're not commonly available. The previous plan references #12 for 6 teeth with a stub profile, which isn't standard. I'll need to explore options for special stub cutters like these for smaller gears."
    },
    {
      "summary": "Clarifying cutter mapping",
      "content": "To achieve accurate cutter mapping for a 6-tooth gear, the #8 cutter for 12-13 teeth could work with a profile shift. But a special stub cutter like #14 might still be necessary for a 6-tooth. The earlier plan referenced #12 for 6 teeth and #7 for 12 teeth. I need to list cutters #1 to #8, considering the stub cutter option for smaller gears. I should check available cutters from vendors like Niagara Cutter or Amazon and provide part numbers and price ranges."
    }
  ],
  "source_analysis_msg_id": "3f1b1027-3251-4606-b08c-2b2d41dca890"
}
```
...
### Updated plan — keyed stack, room-temperature press, and cutter shopping list  

---

#### 1 Assembly changes (no freeze / hot-plate)

| Item | New spec |
|------|----------|
| **Fit class** | *Light drive* 0.0003 – 0.0004 in interference (H7/p6 style) between 0.5000 in shaft shoulders and 0.5003–0.5004 in gear bores. This is tight enough to seat with a 1-ton arbor press at room temp but can still be dismantled with a three-jaw puller if you ever need to replace a gear. |
| **Keying** | Continuous ⅛ in × ⅛ in 1018 key, broached *after* the cone is fully stacked. Broach from the small-end journal toward the large end so chips eject freely; use the witness hole in each gear (drilled during blank turning) to align the keyway angularly. |
| **Retaining compound** | Apply Loctite 638 to key and bore while stacking; it fills any micro-fretting gaps once cured (24 h). |
| **Press sequence** | 1️⃣ Stake a thin stop-collar on the shaft tip.<br>2️⃣ Press gears largest → smallest, shimming each 0.020 in brass washer.<br>3️⃣ Verify axial growth every 5 gears with a calliper (target pitch 0.230 ± 0.001 in).<br>4️⃣ Fit the key, drive flush both ends, wipe excess compound.<br>5️⃣ Install PB thrust washers + #4-40 end-screw preload 5 lbf·in. |

*With brass on leaded-steel the coefficient of friction is low; the 0.0003 in crush needs < 800 lbf, well within a benchtop arbor press.*

---

#### 2 32 DP, 20 ° PA involute cutter set to purchase

| Cutter # | Tooth-count range* | Typical Ø × bore | Example SKU (readily stocked USA) | Est. each price |
|----------|-------------------|------------------|-----------------------------------|-----------------|
| **#1** | 55 – 134 (and rack) | 2 ¼ in × 1 in | Toolmex 5-861-268 (MSC) | ≈ US $38 |
| **#2** | 35 – 54 | 2 ¼ in × 1 in | Precise 320-3022 (Penn Tool) citeturn0search0 | ≈ 40 |
| **#3** | 26 – 34 | 2 ¼ in × 1 in | TMX 5-862-032 (Travers) | ≈ 40 |
| **#4** | 21 – 25 | 2 ¼ in × 1 in | TMX 5-862-033 (Travers) citeturn0search7 | ≈ 40 |
| **#5** | 17 – 20 | 2 ¼ in × 1 in | Precise 320-3025 (Penn Tool) | ≈ 42 |
| **#6** | 14 – 16 | 2 ¼ in × 1 in | TMX 5-862-035 (MSC) | ≈ 42 |
| **#7** | 12 – 13 | 2 ¼ in × 1 in | RDX 13900-32-7 (WT Tool) citeturn0search3 | ≈ 45 |
| **#8** | 10 – 11 | 2 ¼ in × 1 in | Precise 320-3028 (Penn Tool) citeturn0search1 | ≈ 48 |
| **Stub / special** | 6- tooth pinion | 2 ¼ in × 1 in, 20 ° PA **stub** form (#14) | QTC Gear CUT-20-32-14 (SDP/SI) | ≈ 65 |

\*Ranges are for 20 ° full-depth form-relieved cutters per standard AGMA table.  

Total kit cost ≈ US $400; buying the eight common cutters (#1-#8) covers every gear ≥ 10 T, and the #14 stub takes care of the 6-tooth pinion without under-cut. All are HSS, resharpenable 6–8×.

---

#### 3 Process recap with new constraints

1. **Turn blanks** as before; bore 0.5003–0.5004 in.  
2. **Milling** unchanged: divide head 40 : 1, cutter mapping per table.  
3. **Deburr & inspect** (aim ≤ 0.001 in TIR).  
4. **Stack on shaft** with light-drive press, *no thermal steps.*  
5. **Broach ⅛ in keyway** down the finished cone in one pass; deburr key slot ends.  
6. **Final check**: rim run-out ≤ 0.001 in, axial stack ≤ 0.002 in over 4.6 in length.

---

#### 4 Next actions for you

* Confirm the arbor-press tonnage you have (≥ 1 ton recommended).  
* Order cutters + ⅛ in HSS keyway broach & ½ in pilot bushing.  
* Prepare sacrificial aluminium shoes for pressing to avoid marring gear faces.  
* Update your Fusion CAM to omit hot-plate/cryogenic steps in job instructions.

Let me know when the tooling arrives and we’ll generate the cutter-specific feeds & depth-of-cut chart along with the broaching fixture sketch.
