...
Across eight 93-drawing fleet passes run 2026-07-27/28 (PRs #435, #436, #437,
#440), SolidWorks crashed **twice** — roughly 2 in ~750 drawing builds:
...
Different drawings, different operations, no shared recipe code. **Both
recovered automatically** and the pass completed: the watchdog detected the new
...
in ANY of the eight logs, and GDI exhaustion presents as a modal + hang, not a
...
restart does not fix it). Also distinct from a **hung window**, which is
log-only by design and appeared once (`spring_hook`, 15 s, inside
`drawing.curate_dimensions`).
...
is SolidWorks instability the watchdog exists to absorb — not a regression to
chase. Two crashes on the SAME drawing or the SAME operation would be a
different signal and worth investigating.
