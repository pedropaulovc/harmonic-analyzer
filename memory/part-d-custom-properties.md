...
- **Source of truth:** `cad/config/parts.yaml` — 85-part registry (number MHA-###, material, tolerance_class, fit_class, process, confidence), merged over a `defaults:` block. `_config.parts(stem)` returns the merged record.
...
Remaining plan work: Part F (testability suite + engage/disengage config enums) — see [[harmonic-analyzer-project]], [[dof-refactor]].
