...
description: How to run cad/comparisons/bench for the Opus subject — source geometry from a release, regenerate renders, run at --effort high, push partials
metadata:
  type: project
---

Running `cad/comparisons/bench` (pose-presentation benchmark, cad/docs/pose-presentation-benchmark.md) for the **Opus** subject on a Blender-only seat (no SolidWorks / empty `cad/out`).
...
**Regenerate the 270 stimuli** (out/ is gitignored, cleaned after codex): `uv run cad/comparisons/bench/gen_cases.py` (6 first-pass pairs × 45). Blender loading the ~131-STL assembly takes several minutes before the first render lands.
...
- **Comparability pin.** Regenerating stimuli on current `main` is NOT
  comparable: `cad/comparisons/manifest.json`, `tools/composite.py` and
  `render_offline.py` all drifted (camera-pose refreshes). Pin geometry to the
  **v0.19.0** release zip and `cad/comparisons/{manifest.json,tools}` +
