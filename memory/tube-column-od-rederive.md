...
Volume_check tolerances all derive from the constants → self-consistent, no hardcoded volumes to touch. **SW-rebuilt + validated 2026-06-19** (commit 402756af, "regenerate renders + BOM from the green full build" — all five assemblies pass DOF + interference, top fully-defined). *(This line originally read "NOT yet SW-rebuilt"; the rebuild completed the same day.)* Same family of re-anchor as [[od-62mm-reanchor]] (measure-from-photo supersedes legacy). See [[harmonic-analyzer-project]], [[solidworks-modeling-pitfalls]], [[comparison-camera-refinement]].
