...
**Symptom:** the assembly build fails at a mate with `Failed to select mate entity N (AXIS/PLANE at '<Datum>@<part>-1')` — e.g. `crank axis@cone-swing-platform-1`, `CrankAxisSeat@cone-swing-platform-1`, `HandleSeat@crank-arm-1`, `SetScrewAxis@pen-frame-1`. Looks like a mate-code bug; it is actually a **stale part** on disk built before that named datum (`create_axis`/`create_plane` + `name_last_feature`) was added to the part's build script. A part can be *partially* stale (had some named datums but not the newest). (NB: the frame subassembly no longer mates to ANY named datum — every member is placed at its exact transform + one lock mate on a DEFAULT plane — so this symptom shows up in the drive-train/channel/pen/etc. assemblies, not the frame.)

**Fix:** rebuild the offending part(s) directly (e.g. `build_cone_swing_platform.py`, `build_crank_arm.py`, `build_pen_frame.py`), confirming the datum feature is created (`create_axis <Name> OK` → `feature 'AxisN' -> '<Name>'`), then re-run the assembly. Iterate per referenced datum, or just rebuild all of the assembly's parts up front.
